package com.kihlberg.framework.astronomy.calculator;

import com.kihlberg.framework.interfaces.UTCDateTime;
...
public class SunMoonCalculator {
    ...
    public UTCDateTime utcSunRise, utcSunSet;
    public double degSunAz, degSunEl;
    ...
    public UTCDateTime utcMoonRise, utcMoonSet;
    public double degMoonAz, degMoonEl;
    /**
     * Main constructor for Sun/Moon calculations. Time should be given in
     * Universal Time (UT), observer angles in radians.
     * @param dateTime Date and time for the observer.
     ...
     */
    public SunMoonCalculator(UTCDateTime dateTime,
        ...
        double dayFraction = (dateTime.hour + (dateTime.minute + (dateTime.second / 60.0)) / 60.0) / 24.0;
        ...
        TTminusUT = 0;
        if (dateTime.year > -600 && dateTime.year < 2200) {
            double x = dateTime.year + (dateTime.month - 1 + dateTime.day / 30.0) / 12.0;
            double x2 = x * x, x3 = x2 * x, x4 = x3 * x;
            if (dateTime.year < 1600) {
    ...
    public void calcSunAndMoon() {
        ...
        sunAz = out[0];
        degSunAz = sunAz * RAD_TO_DEG;
        sunEl = out[1];
        degSunEl = sunEl * RAD_TO_DEG;
        sunRise = out[2];
        sunSet = out[3];
        sunTransit = out[4];
        ...
        out = doCalc(getMoon());
        moonAz = out[0];
        degMoonAz = moonAz *RAD_TO_DEG;
        moonEl = out[1];
        degMoonEl = moonEl *RAD_TO_DEG;
        moonRise = out[2];
        moonSet = out[3];
        moonTransit = out[4];
     ...
     */
    public static UTCDateTime getDate(double jd) {
/*        if (jd < 2299160.0 && jd >= 2299150.0)
        {
            throw new Exception("invalid julian day " + jd + ". This date does not exist.");
        }
